...
export default function HostGuideModal({ onClose }) {
  ...
  const handleDownloadDetail = () => {
    const stepsFI = [
      ...
      ['Skannausmuoto', 'Jarjestetty: pelaajien on skannattava pisteet jarjestyksessa (1->2->3->...). Satunnainen: pelaajat voivat skannata missa tahansa jarjestyksessa.'],
      ...
      ['Jaa kartta ja koodi', 'Anna pelaajille kartta, jossa nakyy tarkistuspisteiden sijainnit. Jaa 6-numeroinen pelikoodi pelaajille.'],
      ['Aloita ja seuraa', 'Siirry kojelautaan ja paina Aloita. Seuraa reaaliajassa missa pisteessa kukin pelaaja on ja heidän sijoitustaan.'],
    ];
    const stepsEN = [
    ...
    const renderSection = (title, steps, startY) => {
      let y = startY;
      ...
      steps.forEach(([stepTitle, desc], i) => {
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(11);
        doc.setTextColor(30, 30, 30);
        doc.text(`${i + 1}.  ${stepTitle}`, margin, y);
        y += 6;
    ...
    let y = 20;
    y = renderSection('Ohjeet hostille', stepsFI, y);
    y += 8;
    doc.setDrawColor(220, 220, 220);
    doc.line(margin, y, pageW - margin, y);
    y += 10;
    renderSection('Host Guide', stepsEN, y);

    doc.save('Host-Guide.pdf');
  };
